      ...
      const ventaRequest = await pool
        ...
        .query(`
          SELECT 
            ...
            v.cerrada,
            v.efectivo_recibido,
            cc.saldo_actual AS saldo_credito
          FROM Venta v
          JOIN clientes c ON v.id_cliente = c.id_cliente
          JOIN Personal p ON v.id_personal = p.id_personal
          LEFT JOIN Cliente_Credito cc ON v.id_cliente = cc.id_cliente
      ...
      const detallesRequest = await pool
        ...
        .query(`
          SELECT 
            ...
            dv.subtotal,
            CASE
              WHEN dv.subtotal / dv.cantidad = pr.precio_completo THEN 'completo'
              WHEN dv.subtotal / dv.cantidad = pr.precio_medio THEN 'medio'
              WHEN dv.subtotal / dv.cantidad = pr.precio_mayorista THEN 'mayorista'
            END AS tipo_precio
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (tipo_pago === "efectivo" && (efectivo_recibido === undefined || efectivo_recibido <= 0)) {
      return NextResponse.json({ success: false, error: "Efectivo recibido is required for cash payments" }, { status: 400 });
    }

    // Consulta para obtener el tipo de cliente y la ruta
    const customerQuery = `
      SELECT c.tipo_cliente, r.id_ruta 
      FROM clientes c
      ...
      if (tipo_cliente.includes("mayorista")) {
        if (detalle.tipo_precio !== "mayorista") {
          return NextResponse.json(
            { 
              success: false, 
              error: "Los clientes mayoristas solo pueden comprar con precio mayorista" 
            }, 
            { status: 400 }
          );
        }
    ...
    try {
      ...
      const insertVentaQuery = `
        INSERT INTO Venta (
          id_cliente, id_personal, fecha_venta, total, tipo_pago, cerrada, efectivo_recibido
        )
        OUTPUT INSERTED.id_venta
        VALUES (
          @id_cliente, @id_personal, GETDATE(), 0, @tipo_pago, 0, @efectivo_recibido
        )
      `;
      ...
      for (const detalle of detalles_venta) {
        ...
        const productQuery = `
          SELECT precio_completo, precio_medio, precio_mayorista 
          FROM Producto 
        ...
        await inventoryRequest
          ...
          .query(`
            ...
            WHERE id_ruta = @ruta_id AND id_producto = @inv_producto_id
          `);
      }
      ...
      if (tipo_pago === "credito") {
        if (tipo_cliente.includes("credito")) {
          ...
          await creditRequest
            ...
            .query(`
              MERGE INTO Cliente_Credito AS target
              USING (SELECT @cliente_id AS id_cliente, @saldo_actual AS saldo_actual) AS source
              ON target.id_cliente = source.id_cliente
              WHEN MATCHED THEN
                UPDATE SET 
                  saldo_actual = target.saldo_actual + source.saldo_actual,
                  fecha_actualizacion = GETDATE()
              WHEN NOT MATCHED THEN
                INSERT (id_cliente, saldo_actual, fecha_actualizacion)
                VALUES (source.id_cliente, source.saldo_actual, GETDATE());
            `);
        } else {
      ...
      await visitRequest
        ...
        .query(`
          INSERT INTO Estado_Visita (
            id_venta, id_cliente, estado, fecha_visita
          )
          VALUES (
            @visita_venta_id, @visita_cliente_id, @estado_visita, GETDATE()
          )
        `);
      ...
      // Manejo de facturación si es pago en efectivo
      if (tipo_pago === "efectivo") {
        ...
        await invoiceRequest
          ...
          .query(`
            DECLARE @cai_id INT;
            SELECT TOP 1 @cai_id = id_cai FROM CAI WHERE activo = 1;
            
            INSERT INTO Factura (
              id_venta, id_cai, numero_factura, fecha_emision, monto_total, anulada
            )
            VALUES (
              @factura_venta_id, 
              @cai_id, 
              FORMAT(GETDATE(), 'yyyyMMdd') + '-' + CAST(@factura_venta_id AS VARCHAR),
              GETDATE(), 
              @monto_total_factura, 
              0
            )
  ...
  try {
    ...
    try {
      ...
      const currentDetails = await new sql.Request(transaction)
        .input("id_venta", sql.Int, id_venta)
        .query(`
          SELECT dv.id_producto, dv.cantidad, p.precio_completo, p.precio_medio, p.precio_mayorista
          FROM Detalle_Venta dv
      ...
      for (const detalle of currentDetails.recordset) {
        const cantidadARevertir = detalle.subtotal / detalle.precio_completo === detalle.cantidad ? 
          detalle.cantidad : 
          detalle.cantidad * 0.5;
        ...
        await new sql.Request(transaction)
          ...
          .query(`
            ...
            WHERE id_ruta = @ruta_id AND id_producto = @producto_id
          `);
      }
      ...
      if (tipo_pago === "credito" && tipo_cliente.includes("credito")) {
        await new sql.Request(transaction)
          ...
          .query(`
            MERGE INTO Cliente_Credito AS target
            USING (SELECT @cliente_id AS id_cliente, @monto AS saldo_actual) AS source
            ON target.id_cliente = source.id_cliente
            WHEN MATCHED THEN
              UPDATE SET 
                saldo_actual = target.saldo_actual + source.saldo_actual,
      ...
      // 9. Actualizar factura si es pago en efectivo
      if (tipo_pago === "efectivo") {
        await new sql.Request(transaction)
          .input("venta_id", sql.Int, id_venta)
          .input("monto", sql.Decimal(18, 2), nuevoTotal)
