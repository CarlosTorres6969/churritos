...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (monto_total <= 0) {
      return NextResponse.json({ success: false, error: "El monto total debe ser positivo" }, { status: 400 })
    }
    ...
    try {
      ...
      if (ventaExists.recordset.length === 0) {
        ...
        return NextResponse.json({ success: false, error: "La venta especificada no existe" }, { status: 404 })
      }
      ...
      const caiResult = await caiRequest.query(
        `SELECT TOP 1 id_cai, codigo_cai, rango_inicial, rango_final, siguiente_numero FROM CAI WHERE activo = 1 AND GETDATE() BETWEEN fecha_inicio AND fecha_fin AND siguiente_numero <= rango_final ORDER BY fecha_inicio DESC`,
      )
      ...
      const result = await insertRequest
        ...
        .query(
          `INSERT INTO Factura (id_venta, id_cai, numero_factura, fecha_emision, monto_total, anulada) OUTPUT INSERTED.id_factura, INSERTED.fecha_emision VALUES (@id_venta, @id_cai, @numero_factura, GETDATE(), @monto_total, 0)`,
        )
      ...
      return NextResponse.json({ success: false, error: "Error al generar factura" }, { status: 500 })
    }
  } catch (error) {
